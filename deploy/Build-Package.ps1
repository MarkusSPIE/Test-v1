...
.PARAMETER SkipBuild
    Package the existing build output without rebuilding (also skips the version bump - the
    already-built DLLs still have whatever version was baked in last time).

.PARAMETER Label
    Zip filename suffix. Defaults to the freshly-bumped version (e.g. "Beta-v0.1.01") - override
    only if you want a specific filename.
#>
param(
...
# --- bump product version (patch only - edit deploy\VERSION.txt by hand for a major/minor bump) ---
$versionFile = Join-Path $PSScriptRoot "VERSION.txt"
$versionParts = (Get-Content $versionFile -Raw).Trim().Split('.')
...
if (-not $SkipBuild) {
    $patch++
    Set-Content -Path $versionFile -Value "$major.$minor.$patch" -NoNewline
}

$versionLabel = "Beta v{0}.{1}.{2:D2}" -f $major, $minor, $patch
if (-not $Label) { $Label = "Beta-v{0}.{1}.{2:D2}" -f $major, $minor, $patch }

$versionInfoPath = Join-Path $srcRoot "SpieRibbon\VersionInfo.g.cs"
$versionInfoContent = @"
namespace SpieRibbon
{
    ...
    {
        public const string Label = "$versionLabel";
    }
}
"@
...
4. Start Revit. On the "SPIE" tab, click "SPIE Ribbon".
   Use the settings icon in the title bar to enable the modules you want
   (SPIE Algemeen, Civil, E&I, HVAC, BIM Management). New modules start disabled.
